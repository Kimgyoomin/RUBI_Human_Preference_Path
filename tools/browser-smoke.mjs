...
const base='/RUBI_Human_Preference_Path/';
...
try{
  ...
  for(const [label,viewport] of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
    ...
    assert.equal(await page.locator('#choose-a').isDisabled(),true);
    assert.equal(await page.locator('#choose-b').isDisabled(),true);
    ...
    results.push({label,status:'PASS',viewport,unhandledErrors:errors,diagnostic:await page.locator('#error').innerText()});
    await page.close();
  }
  for(const file of ['vendor/mujoco/mujoco.wasm','vendor/ort/ort-wasm-simd-threaded.wasm']){
    ...
    const bytes=new Uint8Array(await response.arrayBuffer());assert.deepEqual(Array.from(bytes.slice(0,4)),[0,97,115,109],file+' WASM header');
    results.push({asset:file,status:'PASS',bytes:bytes.length});
  }
  console.log(JSON.stringify({status:'PASS',checks:results,realRubiLocomotionTested:false},null,2));
  fs.writeFileSync('artifacts/browser-smoke.json',JSON.stringify(results,null,2));
