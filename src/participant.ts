...
import {BINARY_PROTOCOL as UX_PROTOCOL,BINARY_LABEL_CONDITION as UX_LABEL_CONDITION,routeChoice,isBinaryAnswer,assertIntroAssets,introRecordVersion} from './core/binary-study.ts';
import type {BinaryAnswer} from './core/binary-study.ts';
import {IntroVideo} from './runtime/intro-video.ts';

type Study={id:string;status:'draft'|'released';responseApi:string;bundleBaseUrl:string;scenarios:PilotScene[];guidedScenarios?:PilotScene[];introduction?:{manifestUrl:string}};
type Phase='welcome'|'profile'|'intro'|'loading'|'observe'|'choice'|'saving'|'complete';
type Session={id:string;participantId:string;startedAt:string;order:number[];presentations:RouteKey[];index:number;profile?:ProfileAnswers;consent:boolean;tutorialCompleted:boolean;complete:boolean;pending?:Record<string,unknown>;responses:Record<string,unknown>[];repeatIndex?:number;previousSessionId?:string;introduction?:{version:string;contentId:string;acknowledgedAtUtc:string}};
...
<p>먼저 RUBI의 보행 예시를 보고 총 <span id="g-total">16</span>문항에 답합니다. 이름이나 이메일은 묻지 않습니다. 고른 답, 화면을 본 시간, 로봇의 움직임, 시작 전 질문의 답변을 함께 저장합니다.</p>
...
<div class="g-actionbar"><p id="g-action-hint"></p><div class="g-watch-actions"><button id="g-watch" class="g-primary">A 보기</button><button id="g-enough" class="g-primary" disabled hidden>이 정도면 알겠어요</button><button id="g-stop" class="g-secondary" hidden>보기 멈추기</button></div></div></section>
...
function show(next:Phase){
  phase=next;if(next!=='intro')introduction.pause();
  ...
  if(next==='observe'||next==='choice')viewer?.resize();updateChoices();
}
...
function readyPath(key:RouteKey){
  active=key;show('observe');$('g-counter').textContent=`${state.index+1} / ${scenes.length} 문항`;
...
function nextTrial(){
  ...
  aKey=state.presentations[state.index];bKey=aKey==='direct'?'detour':'direct';runs={};seen=new Set();
...
async function choose(choice:BinaryAnswer){
  ...
  try{
    ...
    const payload={schemaVersion:1,consent:true,submissionId:crypto.randomUUID(),participantId:state.participantId,sessionId:state.id,
      experimentId:study.id,studyStatus:study.status,protocolVersion:UX_PROTOCOL,trialId:`binary-v6-${state.index}-${scenario.id}`,trialSequence:state.index+1,isPractice:false,
      ...
      labelConditionVersion:UX_LABEL_CONDITION,observationPolicy:OBSERVATION_POLICY,choiceDuringReplay:duringReplay,queryReason:choice==='skip'?'fixed_pilot_information_skip':'fixed_pilot_informed_binary',
...
async function sendPending(){
  ...
  try{
    ...
    if(!state.responses.some(r=>r.submissionId===p.submissionId))state.responses.push(p);state.pending=undefined;state.index++;save();$('g-save-status').textContent='답변이 저장되었습니다.';
    if(state.index>=scenes.length)await finalize();else{await new Promise(r=>setTimeout(r,350));if(engine&&viewer&&networks)nextTrial();else await prepare();}
...
async function finalize(){
  ...
  try{
    ...
    const reply=await collector!.request('profile',{experimentId:study.id,studyStatus:study.status,sessionId:state.id,participantId:state.participantId,profileSchemaVersion:'pre-profile-plain-v2',
    ...
    if(reply.kind!=='profile'||reply.submissionId!==state.id)throw new Error('시작 전 질문의 답변을 저장하지 못했습니다. 다시 시도해 주세요.');state.complete=true;save();show('complete');
...
function freshSession(participantId:string,previous?:Session):Session{
  return {id:crypto.randomUUID(),participantId,startedAt:new Date().toISOString(),order:pilotOrder(scenes),presentations:scenes.map(()=>crypto.getRandomValues(new Uint8Array(1))[0]%2?'direct':'detour'),index:0,consent:false,tutorialCompleted:false,complete:false,responses:[],repeatIndex:previous?(previous.repeatIndex??0)+1:0,previousSessionId:previous?.id};
}
function restart(){
...
$<HTMLFormElement>('g-profile-form').onsubmit=e=>{
  ...
  if(!validProfile(p))return;try{state.profile=p;save();void showIntro();}catch(error){fail(error);}
};
...
async function bootstrap(){
  try{
    ...
    state=valid?restored!:freshSession(participantId);save();if(state.complete){show('complete');return;}if(state.index===scenes.length){show('saving');await finalize();return;}
